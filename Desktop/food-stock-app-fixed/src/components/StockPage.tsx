...
const StockPage: React.FC<Props> = ({
  ...
  // filter out 'def/prep' for expiry warning
  useEffect(() => {
    const count = items
      .filter(i => i.category !== 'def/prep')
      .filter(i => new Date(i.expiry) <= soonCutoff).length;
    setExpiringSoonCount(count);
  }, [items]);

  const catList = [{ label: 'All', value: 'all', emoji: '📦' }, ...categories];

  // filter & search (exclude 'def/prep')
          ...
          >
            ...
            {c.value !== 'all' && (
              <span className="ml-1 text-gray-200">
                ({items.filter(i => i.category === c.value && i.category !== 'def/prep').length})
              </span>
            )}
          </button>
        ))}
